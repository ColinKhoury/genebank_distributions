...
country_regions <- read_excel('D:/Tobackup/_hmcfw/Genebank_distributions/inputs/countries_regions/Countries_gb_data_regions_2016_1_9_canussep.xlsx', sheet=1); country_regions <- as.data.frame(country_regions)
data_country <- read_excel("D:/Tobackup/_hmcfw/Genebank_distributions/inputs/AAFC/AAFC_Origincountry-Tocountry_circosfinal_2016_2_4_avannual_allfromCan.xlsx", sheet=1); data_country <- as.data.frame(data_country)
...
# Guardar
write.csv(data_country, "D:/Tobackup/_hmcfw/Genebank_distributions/inputs/AAFC/Prod_items_country_regions_uscansep_allfromCan.csv", row.names=FALSE)
rm(country_regions, data_country)

g=gc(); rm(list=ls())

# Leerlo
data_exp <- read.csv("D:/Tobackup/_hmcfw/Genebank_distributions/inputs/AAFC/Prod_items_country_regions_uscansep_allfromCan.csv")
...
for(i in 1:nrow(transferences))
{
  ...
  data_country[[i]] <- data.frame(Region_origin=transferences$Region_origin[i], Region_recipient=transferences$Region_recipient[i],Average=sum(subdata_exp[,4]))
}; rm(i, subdata_exp, data_exp)

data_country <- Reduce(function(...){rbind(..., deparse.level = 1)}, data_country)
write.csv(data_country, "D:/Tobackup/_hmcfw/Genebank_distributions/inputs/AAFC/regions_sourceofprod_uscansep_allfromCan.csv",row.names=FALSE)
...
cOrgData_region <- read.csv("D:/Tobackup/_hmcfw/Genebank_distributions/inputs/AAFC/regions_sourceofprod_uscansep_allfromCan.csv")
...
regions <- read.csv('D:/Tobackup/_hmcfw/Genebank_distributions/inputs/regions_24list_uscansep.csv')
regions <- as.character(sort(unique(regions$Region)))
...
cOrgData_region$R_recipients <- gsub(pattern=regions[1], replacement='And', cOrgData_region$R_recipients, fixed=TRUE)
cOrgData_region$R_recipients <- gsub(pattern=regions[2], replacement='ANZ', cOrgData_region$R_recipients, fixed=TRUE)
cOrgData_region$R_recipients <- gsub(pattern=regions[4], replacement='Car', cOrgData_region$R_recipients, fixed=TRUE)
cOrgData_region$R_recipients <- gsub(pattern=regions[5], replacement='C\nAfr', cOrgData_region$R_recipients, fixed=TRUE)
cOrgData_region$R_recipients <- gsub(pattern=regions[6], replacement='C\nAm', cOrgData_region$R_recipients, fixed=TRUE)
cOrgData_region$R_recipients <- gsub(pattern=regions[7], replacement='C\nAsia', cOrgData_region$R_recipients, fixed=TRUE)
cOrgData_region$R_recipients <- gsub(pattern=regions[8], replacement='E\nAfr', cOrgData_region$R_recipients, fixed=TRUE)
cOrgData_region$R_recipients <- gsub(pattern=regions[9], replacement='E\nAsia', cOrgData_region$R_recipients, fixed=TRUE)
cOrgData_region$R_recipients <- gsub(pattern=regions[11], replacement='NE\nEur', cOrgData_region$R_recipients, fixed=TRUE)
...
cOrgData_region$R_recipients <- gsub(pattern=regions[24], replacement='W\nAsia', cOrgData_region$R_recipients, fixed=TRUE)

cOrgData_region$R_origin <- gsub(pattern=regions[1], replacement='And', cOrgData_region$R_origin, fixed=TRUE)
cOrgData_region$R_origin <- gsub(pattern=regions[2], replacement='ANZ', cOrgData_region$R_origin, fixed=TRUE)
cOrgData_region$R_origin <- gsub(pattern=regions[4], replacement='Car', cOrgData_region$R_origin, fixed=TRUE)
cOrgData_region$R_origin <- gsub(pattern=regions[5], replacement='C\nAfr', cOrgData_region$R_origin, fixed=TRUE)
cOrgData_region$R_origin <- gsub(pattern=regions[6], replacement='C\nAm', cOrgData_region$R_origin, fixed=TRUE)
cOrgData_region$R_origin <- gsub(pattern=regions[7], replacement='C\nAsia', cOrgData_region$R_origin, fixed=TRUE)
cOrgData_region$R_origin <- gsub(pattern=regions[8], replacement='E\nAfr', cOrgData_region$R_origin, fixed=TRUE)
cOrgData_region$R_origin <- gsub(pattern=regions[9], replacement='E\nAsia', cOrgData_region$R_origin, fixed=TRUE)
cOrgData_region$R_origin <- gsub(pattern=regions[11], replacement='NE\nEur', cOrgData_region$R_origin, fixed=TRUE)
...
cOrgData_region$R_origin <- gsub(pattern=regions[24], replacement='W\nAsia', cOrgData_region$R_origin, fixed=TRUE)
rm(regions)
...
order.regions <- c(5,24,9,3,19,1,4,10,21,12,15,13,23,11,20,22,16,17,14,7,6,2,8,18)
...
write.csv(origin_regions,paste("D:/Tobackup/_hmcfw/Genebank_distributions/inputs/AAFC/interchange_AAFC_avannaul_canussep1_allfromCan.csv",sep=""),row.names=TRUE)

origin_regions <- read.csv("D:/Tobackup/_hmcfw/Genebank_distributions/inputs/AAFC/interchange_AAFC_avannaul_canussep1_allfromCan.csv", row.names = 1)
...
n <- nrow(df1)
# Include RGB color
colors <- read.csv("D:/Tobackup/_hmcfw/Genebank_distributions/inputs/id_colors_regions_AAFC.csv")
...
colors$region <- gsub(pattern='Andes', replacement='And', x=colors$region, fixed=TRUE)
colors$region <- gsub(pattern='C\\nAmer', replacement='C\nAm', x=colors$region, fixed=TRUE)
...
df1$xmax[which(df1$region=="C\nAm")] <- 0.1
df1$xmax[which(df1$region=="Car")] <- 0.1
df1$xmax[which(df1$region=="Tr. S\nAm")] <- 0.1
df1$xmax[which(df1$region=="W\nAfr")] <- 0.1
df1$xmax[which(df1$region=="C\nAfr")] <- 0.1
df1$xmax[which(df1$region=="IOI")] <- 0.1
df1$xmax[which(df1$region=="C\nAsia")] <- 0.1
df1$xmax[which(df1$region=="Pac")] <- 0.1
##
##plot sectors
##
library("circlize")
...
circos.trackPlotRegion(ylim = c(0, 1), factors=df1$region, track.height=0.1, bg.border = NA, bg.col = NA, bg.lty =0, bg.lwd=0.0001,
                       ...
                       panel.fun = function(x, y) {
                         ...
                         circos.text(x=mean(xlim), y=1.5, labels=name, facing='inside', cex=1, col=df1$rcol[z], font=2, niceFacing=TRUE) # facing = dd, adj = aa, bending.inside
...
#to eliminate 0.1 ghost flows I added the 0.55 thresh below
...
dev.copy2pdf(file=paste("D:/Tobackup/_hmcfw/Genebank_distributions/inputs/AAFC/interchange_AAFC_avannual_all_canussep1b_allfromCane.pdf",sep=""), height=10, width=10)
